...
# user name can only consist of lower letters and underscore, more than 3 characters
username_re = re.compile(r'^[a-z0-9][a-z0-9_]{2,}$', re.UNICODE)
...
class UserNameField(StrippedNonEmptyCharField):
    def __init__(self, db_model=User, db_field='username', must_exist=False, skip_clean=False, label=_('choose a username'), **kw):
        ...
        error_messages = {'required':_('user name is required'),
                        ...
                        'invalid':_('user name can only consist of lower letters underscore, more than 3 characters'),
                        'toshort':_('user name is too short, please use at least %d characters') % settings.MIN_USERNAME_LENGTH
                    }
